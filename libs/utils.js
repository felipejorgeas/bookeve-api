module.exports = function (nodemailer, htmlToPdf) {
    var Utils = {
        ...
        setResponse: function (error, result) {
            this.timeResponse.timef = new Date().getTime();
            var time = new Date(this.timeResponse.timef - this.timeResponse.timei) / 1000;
            var response = {
                status: 0,
                message: '',
                time: time,
            ...
            switch (error) {
                ...
                case 200:
                    ...
                    break;
                case 201:
                    response.message = 'Executado com sucesso mas sem dados a retornar!';
                    break;
                default:
                    response.status = 0;
        ...
        sendMail: function (to, subject, message, from, attachments, data, callback) {
            ...
            var transporter = nodemailer.createTransport({
                service: 'Gmail',
                auth: {
                    user: 'felipejorgeas@gmail.com',
                    pass: '93793272'
                },
                debug: true
            }, {
                    from: (from ? from : 'felipejorgeas@gmail.com'),
                    headers: {
